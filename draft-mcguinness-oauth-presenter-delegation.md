...
abbrev: "OAuth Presenter Delegation"
category: std

docname: draft-mcguinness-oauth-presenter-delegation-latest
submissiontype: IETF
number:
date:
v: 3
...
venue:
  ...
  github: "mcguinness/draft-mcguinness-oauth-presenter-delegation"
  latest: "https://mcguinness.github.io/draft-mcguinness-oauth-presenter-delegation/draft-mcguinness-oauth-presenter-delegation.html"
...
Proof-of-possession (PoP) binding transforms a bearer security token into one that only its intended holder can use.  A JWT carries a confirmation (`cnf`) claim {{RFC7800}} identifying a key; the presenter proves possession of that key (for example, with DPoP {{RFC9449}} or mutual TLS) when using the token.  OpenID Connect Key Binding {{OpenID.KeyBinding}} applies this to the ID Token, and JWT access tokens {{RFC9068}} and Token Exchange {{RFC8693}} outputs commonly carry `cnf` as well.

PoP binding creates a difficulty whenever the party that should present a token is not the party the token is bound to.  This arises across the delegation space:

*  In cross-client delegation {{I-D.mcguinness-oauth-cross-client-delegation}}, one client (the Delegate) presents an Identity Assertion issued to, and key-bound to, a different client (the Initiator).  The Delegate does not hold the Initiator's key.
*  In presenter-transition and rebind flows within the OAuth Actor Profile {{I-D.mcguinness-oauth-actor-profile}}, a token bound to one presenter must be continued by another.
*  More generally, any Token Exchange in which the requesting party differs from the party a key-bound `subject_token` is bound to faces the same gap.
...
*  How the confirmation-key holder decides to delegate, and any user interface or consent around that decision.
*  Establishing trust in the source token itself; that is the responsibility of the token's own profile and its issuer.
*  Cross-trust-domain validation in which the verifier cannot obtain or trust the source token's issuer and confirmation key; see {{security-domains}}.
...
Unless otherwise specified, OAuth and JOSE terms are used as defined in {{RFC6749}}, {{RFC7515}}, {{RFC7517}}, {{RFC7519}}, {{RFC7800}}, and {{RFC9449}}.

The following terms are used in this document:

Source Token:
: A security token, carrying an RFC 7800 confirmation (`cnf`) claim, whose presentation authority is being delegated.  For example, a key-bound ID Token {{OpenID.KeyBinding}} or a `cnf`-bearing access token.
...
# The Presenter Delegation Assertion {#pda}

A Presenter Delegation Assertion is a JWT {{RFC7519}} secured as a JWS {{RFC7515}}.

## JOSE Header {#pda-header}
...
To use a Source Token under this mechanism, a presenter conveys, by whatever means the consuming protocol defines:
...
This mechanism is used with OAuth 2.0 Token Exchange {{RFC8693}} to let a party that is not the party a key-bound `subject_token` is bound to obtain a token, under authority delegated by the confirmation-key holder.
...
A captured copy of the Initiator's ID Token is useless to an attacker without both a PDA signed by `K_init` and possession of `K_del`.
